...
// buildRequestURL constructs the full request URL, honoring RequestPathOverrides from the
// custom provider config (see providerUtils.GetRequestPath) instead of only a context override.
func (provider *MistralProvider) buildRequestURL(ctx context.Context, defaultPath string, requestType schemas.RequestType) string {
