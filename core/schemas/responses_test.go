...
// TestResponsesMessageMCPListToolsRoundTrip guards against two bugs on the
// mcp_list_tools item type:
//
//  1. A field collision: ResponsesToolMessage.Tools (tool_search_output) and
//     the embedded ResponsesMCPListTools.Tools (mcp_list_tools) both use the
//     wire key "tools". Struct-tag-based encoding can't disambiguate two
//     same-named fields at different embedding depths — the shallower one
//     would silently win for every message type and the other would never
//     survive marshal/unmarshal.
//  2. A pre-existing, unrelated decoder limitation (reproduces identically on
//     a clean upstream/dev checkout): ResponsesMCPListTools sits behind two
//     levels of anonymous pointer embedding (ResponsesMessage ->
//     *ResponsesToolMessage -> *ResponsesMCPListTools), and neither
//     unmarshal nor marshal auto-vivifies/promotes through that second level,
//     so none of ResponsesMCPListTools' own fields (e.g. ServerLabel) ever
//     reach the wire unless routed manually — same as `tools` above.
func TestResponsesMessageMCPListToolsRoundTrip(t *testing.T) {
	raw := `{"type":"mcp_list_tools","server_label":"grafana","tools":[{"name":"query_prometheus","input_schema":{"type":"object"}}]}`
	...
	if msg.ResponsesMCPListTools.ServerLabel != "grafana" {
		t.Fatalf("expected server_label to survive unmarshal, got %q", msg.ResponsesMCPListTools.ServerLabel)
	}
