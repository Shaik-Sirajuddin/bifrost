...
func (s *BifrostHTTPServer) OnKeyAdded(ctx context.Context, provider schemas.ModelProvider, key schemas.Key) error {
	...
	// A newly-added disabled key has nothing to discover yet — the keyconfig
	// snapshot above still reflects it (as disabled), but skip the network
	// call so we don't fetch models against a key the operator just turned off.
	if isKeyEnabled(key) {
...
func (s *BifrostHTTPServer) OnKeyUpdated(ctx context.Context, provider schemas.ModelProvider, key schemas.Key) error {
	...
	s.Config.ModelCatalog.InvalidateLive(provider, keyID)
	// If the key was just disabled, drop its cached entries (done above) but
	// don't refetch — a disabled key should not be queried.
	if isKeyEnabled(key) {
		s.Config.ModelCatalog.MarkLiveRefreshInFlight(provider)
		s.FetchAndStoreLiveForKey(ctx, provider, keyID)
		s.Config.ModelCatalog.NoteLiveRefreshCompleted(provider)
...
// isKeyEnabled reports whether a key is active. Enabled defaults to true
// (nil means the DB default of true) so only an explicit false disables it.
func isKeyEnabled(key schemas.Key) bool {
